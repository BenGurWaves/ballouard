/**
 * GET /admin/client/:token — Admin-only full brief page.
...
export async function onRequestGet(context) {
  ...
  const socials = (p1.socials||[]).map(s=>`<a href="${esc(s)}" style="color:#C49C7B;font-size:12px;display:block;margin-bottom:4px">${esc(s)}</a>`).join('');
...
body{background:#0D0C09;color:#DEC8B5;font-family:'Inter',sans-serif;-webkit-font-smoothing:antialiased;padding:0}
.topbar{background:rgba(13,12,9,.95);border-bottom:1px solid rgba(222,200,181,.07);padding:14px 40px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:10}
...
.wrap{max-width:800px;margin:0 auto;padding:48px 40px 80px}
.hero-name{font-family:'Instrument Serif',Georgia,serif;font-size:clamp(2.5rem,5vw,4.5rem);font-weight:400;letter-spacing:-.04em;line-height:1;color:#DEC8B5;margin-bottom:8px}
.hero-sub{font-size:.72rem;color:#565250;letter-spacing:.1em;margin-bottom:40px}
.status-badges{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:48px}
...
.divider{height:1px;background:rgba(222,200,181,.07);margin:32px 0}
/* AI briefing block */
.ai-block{background:rgba(196,156,123,.05);border:1px solid rgba(196,156,123,.15);padding:20px;margin-bottom:40px}
.ai-title{font-size:.58rem;letter-spacing:.18em;text-transform:uppercase;color:#C49C7B;margin-bottom:12px}
.ai-text{font-size:.78rem;color:#8a8680;line-height:1.9;white-space:pre-wrap;font-family:'Inter',sans-serif}
</style>
</head>
<body>
<div class="topbar">
...
<div class="wrap">
  <div class="hero-name">${esc(lead.client_name||p1.full_name||'Unnamed')}<span style="color:#C49C7B">.</span></div>
  <div class="hero-sub">${esc(lead.client_email||p1.email||'')} &mdash; Submitted ${lead.submitted_at ? new Date(lead.submitted_at).toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'}) : 'Not yet'}</div>
  ...
  <!-- AI-ready briefing block -->
  <div class="ai-block">
    <div class="ai-title">AI Build Brief — Copy &amp; Paste</div>
    <div class="ai-text">${esc(`CLIENT: ${lead.client_name||p1.full_name||''}
...
ADDITIONAL NOTES: ${p5.additional_notes||''}`)}</div>
  </div>
  ...
  ${section('Business Context', p1.context ? `<tr><td colspan="2" style="font-size:13px;color:#e8ddd3;line-height:1.8;padding:6px 0;white-space:pre-wrap">${esc(p1.context)}</td></tr>` : '')}
  ${section('Target Audience', p2.target_customer ? `<tr><td colspan="2" style="font-size:13px;color:#e8ddd3;line-height:1.8;padding:6px 0;white-space:pre-wrap">${esc(p2.target_customer)}</td></tr>` : '')}
  ...
  ${section('Socials', row('Profiles',(p1.socials||[]).join(', '))+row('Show Icons',p1.include_socials?'Yes':'No'))}
  ...
  ${lead.site_link ? `<div style="margin:0 0 32px"><div style="font-size:10px;letter-spacing:.18em;text-transform:uppercase;color:#C49C7B;margin-bottom:10px">Finished Site</div><a href="${esc(lead.site_link)}" style="color:#C49C7B;font-size:14px">${esc(lead.site_link)}</a></div>` : ''}
</div>
</body></html>`;
