 ...
 * Auth: short-lived view token via ?t= query param (never the permanent secret).
 * Temp tokens are generated by POST /api/admin/temp-token and expire in 5 minutes.
 */
import { getSupabase } from '../../_lib/supabase.js';
import { secureErr } from '../../_lib/security.js';
...
async function validateViewToken(request, env) {
  ...
  // Accept permanent secret via header (for programmatic use)
  if (hdr && env.ADMIN_SECRET) {
    const { timingSafeEqual } = await import('../../_lib/security.js');
    if (await timingSafeEqual(hdr, env.ADMIN_SECRET)) return true;
  }

  // Accept short-lived view token from URL
  if (t && kv) {
...
export async function onRequestGet(context) {
  ...
  const domainDisplay =
    lead.domain_choice==='client_provides' ? 'Client connects own domain' :
    lead.domain_choice==='client_has_needs_setup' ? `Client has domain — Velocity to configure${lead.domain_name?' ('+lead.domain_name+')':''}` :
    lead.domain_choice==='velocity_provides' ? `Velocity to register${lead.domain_name?' — '+lead.domain_name:''}` :
    lead.domain_choice || '—';
